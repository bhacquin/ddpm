...
def clipped_zoom(img, zoom_factor):
    ...
    ch_h = int(np.ceil(h / zoom_factor))
    ch_w = int(np.ceil(w / zoom_factor))

    top_h = (h - ch_h) // 2
    top_w = (w - ch_w) // 2
    img = scizoom(img[top_h:top_h + ch_h, top_w:top_w + ch_w], (zoom_factor, zoom_factor, 1), order=1)
...
def snow(x, severity=1):
    ...
    x = c[6] * x + (1 - c[6]) * np.maximum(x, cv2.cvtColor(x, cv2.COLOR_RGB2GRAY).reshape(shape[0], shape[1], 1) * 1.5 + 0.5)
