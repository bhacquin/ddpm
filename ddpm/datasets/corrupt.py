...
from ddpm.datasets.utils_corruption.image import bilinear_interpolation, smoothstep
from ddpm.datasets.utils_corruption.perlin_noise import PerlinNoiseGenerator
from ddpm.datasets.base import Transform
import abc
...
class Sparkles(Transform):
    ...
    def transform(self, image, centers, radii, nrays, amounts, color, randomness, seed):
        ...
        for center, rays, amount, radius in zip(centers, nrays, amounts, radii):
            ...
            image = np.array([[kernel(np.array([y,x]), image[y,x,:].astype(float), center, radius, ray_lengths, amount, color)\
                for x in range(self.im_size)] for y in range(self.im_size)])
...
class InverseSparkles(Transform):
    ...
    def transform(self, image, center, radius, rays, amount, color, randomness, seed):
        ...
        out = np.array([[kernel(np.array([y,x]), image[y,x,:].astype(float), center, radius, ray_lengths, amount, color)\
                for x in range(self.im_size)] for y in range(self.im_size)])
...
class PerlinNoise(Transform):
    ...
    def sample_parameters(self):
        m = np.array([[1,0],[0,1]]) / (32 * self.im_size / 256)
        turbulence = 16.0
        gain = 0.5
        bias = 0.5
    ...
    def transform(self, image, m, turbulence, seed, gain, bias, alpha):
        ...
        out = (1 - alpha) * image.astype(float) + 255 * alpha * noise
        return np.clip(out, 0, 255).astype(np.uint8)
...
class Technicolor(Transform):
    ...
    def transform(self, image, amount):
        ...
        red = greenmatte * bluematte * image[:,:,0].astype(float)
        green = redmatte * bluematte * image[:,:,1].astype(float)
        blue = redmatte * greenmatte * image[:,:,2].astype(float)
...
class Pseudocolor(Transform):
    ...
    def transform(self, image, color0, color1, color2, color3, color4, smoothness, amount):
        ...
        vals = np.array([0.2126, 0.7152, 0.0722]).reshape(1,1,3)
        luma = np.sum(image.astype(float)*vals, axis=2, keepdims=True)/255
...
class HueShift(Transform):
    ...
    def transform(self, image, amount):
        ...
        hsv_image = rgb2hsv(image.astype(float)/255)
                        ...
                        for j in [-1,0,1]:
                            ...
                            if jx > 0 and jx < self.im_size:
                                ...
                                if w != 0:
                                    ...
                                    curr_val = image_copy[k//self.im_size, k%self.im_size,:].astype(float)
        ...
        for i in range(num_lines):
            ...
            for rx, ry, rv in zip(rxc, ryc, rval):
                if rx >= 0 and ry >= 0 and rx < self.im_size and ry < self.im_size:
                    ...
                    yc.append(ry)
                    val.append(rv)
            xc, yc, val = np.array(xc, dtype=int), np.array(yc, dtype=int), np.array(val)
            out[xc, yc, :] = (1.0 - val.reshape(-1,1)) * out[xc, yc, :].astype(float) + val.reshape(-1,1)*128
...
class PinchAndTwirl(Transform):
    ...
    def sample_parameters(self):
        num_per_axis = 5 if self.im_size==256 else 3
...
class FishEye(Transform):
    ...
    def sample_parameters(self):
        seed = np.random.randint(low=0, high=2**32)
        density = 0.01 * 256**2 / (self.im_size**2)
    ...
    def transform(self, image, density, eta, radius, seed):
        ...
        for i in range(num):
            ...
            l = max(np.floor(center[1]-radius).astype(int), 0)
            r = min(np.ceil(center[1]+radius).astype(int), self.im_size)
            u = max(np.floor(center[0]-radius).astype(int), 0)
            d = min(np.ceil(center[0]+radius).astype(int), self.im_size)
...
class ChromaticAbberation(Transform):
    ...
    def convert_from_numpy(self, numpy_record):
        return {'rgb_shifts' : numpy_record.reshape(3,2).astype(int)}
