...
def get_filtering_metrics(df):
    ...
    metrics = []
    img_info_list = [(row["path"], row["algo"], i % num_gpus) for i, row in enumerate(df.iterrows())]
...
df.loc[
    (
        df['path'].apply(lambda x: 'scitas' in str(x)) &
...
df.to_csv("../metrics_csv/df_with_metrics.csv")
print("DONE")
# print(df.head(2))
# print("\n\n")
...
def save_top_k_by_algo(df, k, ascending, metric_name, col_to_delete, other_cols_to_group=[], file_suffix=None):
    df_with_metrics_filtered = (
        ...
        .apply(lambda x: x.sort_values(metric_name, ascending=ascending).head(k), include_groups=False)
        .reset_index().drop(columns=col_to_delete)
    )
...
save_top_k_by_algo(df, 1, False, "psnr_to_target", "level_4")
save_top_k_by_algo(df, 4, False, "psnr_to_target", "level_4")
save_top_k_by_algo(df, 1, True, "lpips_to_target", "level_4")
save_top_k_by_algo(df, 4, True, "lpips_to_target", "level_4")

save_top_k_by_algo(df, 1, False, "psnr_to_target", "level_6", ["latent", "epsilon"], "by_latent_epsilon")
save_top_k_by_algo(df, 4, False, "psnr_to_target", "level_6", ["latent", "epsilon"], "by_latent_epsilon")
